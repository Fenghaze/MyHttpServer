...
//初始化客户连接：获得客户信息，并添加到epfd
void HTTPConn::init(int epfd, int sockfd, const sockaddr_in &addr)
{
    m_sockfd = sockfd;
    m_address = addr;
    m_epollfd = epfd;
...
//从状态机，解析请求体的每一行：直到读取到回车换行符时，才是完整的一行
